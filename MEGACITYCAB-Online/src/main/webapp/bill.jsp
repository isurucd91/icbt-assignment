...
<body>
	...
	<%
		...
		List<Booking> bookList = bookServ.GetData(logServ.GetUserID(username));
	%>
	<div class="content">
	    <%
		...
		<div id="item">
			<p style="font-weight: bold; text-align:center; text-shadow: 2px 2px 2px rgb(220,220,220);">
				...
				<p style="text-align:center;">Driver ID(Name): <%= book.getmDriverID() %> (<%= logServ.GetUserName(book.getmDriverID()) %>)</p>
				<%
					if (request.getAttribute("billval") != null && request.getAttribute("bid") != null)
					{
						String value = (String)request.getAttribute("billval");
						String bid = (String)request.getAttribute("bid");
						if (book.getmID() == Integer.parseInt(bid))
						{
				%>
				<p style="text-align:center;">Bill Value: <%= value %> RS</p>
				<%
						}
					}
				%>
				<form method="post" style="padding: 10px; margin-bottom: 10px;" action="BillBook">
					<div style="display: flex; justify-content: center; align-items: center">
						<input type="submit" name="<%= book.getmID() %>" class="button" value="Calculate">
					</div>
				</form>
			</p>
		</div>		
